...
func TestVector3(t *testing.T) {
	...
	// Test cross product
	cross := v1.Cross(v2)
	expected = mathf.NewVector3(-3, 6, -3)
	...
	// Test absolute value
	neg := mathf.NewVector3(-1, -2, -3)
	absV := neg.Abs()
	expected = mathf.NewVector3(1, 2, 3)
	if !vec3AlmostEqual(absV, expected) {
		t.Errorf("Abs failed: got %v, expected %v", absV, expected)
	}

	// Test clamp
	min := mathf.NewVector3(0, 0, 0)
	max := mathf.NewVector3(1, 1, 1)
	toClamp := mathf.NewVector3(-1, 2, 0.5)
	clamped := toClamp.Clamp(min, max)
	expected = mathf.NewVector3(0, 1, 0.5)
