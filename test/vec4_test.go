...
func TestVector4(t *testing.T) {
	...
	// Test sign
	sign := neg.Sign()
	expected = mathf.NewVector4(-1, -1, -1, -1)
	if !vec4AlmostEqual(sign, expected) {
		t.Errorf("Sign failed: got %v, expected %v", sign, expected)
	}

	// Test clamp
	min := mathf.NewVector4(0, 0, 0, 0)
	max := mathf.NewVector4(1, 1, 1, 1)
	toClamp := mathf.NewVector4(-0.5, 1.5, 0.5, 2)
	clamped := toClamp.Clamp(min, max)
	expected = mathf.NewVector4(0, 1, 0.5, 1)
