...
func TestVector3i(t *testing.T) {
	...
	// Test absolute value
	neg := mathf.NewVector3i(-1, -2, -3)
	absV := neg.Abs()
	expected = mathf.NewVector3i(1, 2, 3)
	if !vec3iEqual(absV, expected) {
		t.Errorf("Abs failed: got %v, expected %v", absV, expected)
	}

	// Test clamp
	min := mathf.NewVector3i(0, 0, 0)
	max := mathf.NewVector3i(1, 1, 1)
	toClamp := mathf.NewVector3i(-1, 2, 0)
	clamped := toClamp.Clamp(min, max)
	expected = mathf.NewVector3i(0, 1, 0)
